...
DECLARE
    ...
    quota_used int;
BEGIN
    SELECT COALESCE((properties->>'quota_daily')::int, 2), COALESCE((properties->>'quota_used_today')::int, 0)
...
BEGIN
    INSERT INTO generation_requests(
        ...
        gen_random_uuid(), p_user_id, p_task, p_provider, p_provider, p_prompt, p_aspect_ratio, p_quantity, p_status, now(), now(), p_properties
    ) RETURNING id INTO new_job;

    INSERT INTO usage_events(id, user_id, request_id, event_type, success, created_at, properties)
    VALUES (gen_random_uuid(), p_user_id, new_job, p_task, true, now(), jsonb_build_object('provider', p_provider));
